...
export default function StatsCard() {
    ...
    const handleScroll = () => {
      if (!ticking) {
        requestAnimationFrame(() => {
          ...
          const shouldActivate = rect.top < windowHeight * 0.2;

          if (shouldActivate !== allStatsActive) {
  ...
  const revolvingStars = useMemo(() => {
    return stats.map((stat, index) => {
      ...
      // Pre-calculate circular motion keyframes
  ...
  return (
    ...
    >
      {/* Background elements */}
      <div className="absolute top-[15%] left-[-5%] w-3/5 h-2/5 bg-amber-300/20 rounded-full blur-3xl animate-pulse" />
      <div className="absolute bottom-[15%] right-[-10%] w-3/4 h-2/4 bg-blue-300/20 rounded-full blur-3xl animate-pulse" />
